...
!
! Feb 18, 2023
! -- limit r > r0, r0=1.e-6 m
! Feb 19, 2023
! -- set ck=cr=0 in drdt(1); cannot set ck=cr=0 in if-then statement for some reason
! -- set rhol = rhow to use in denom (and hence drdt(1))
! -- r0 = 0.01e-6 m
!
! -------------------------------------------------------------------------------------------------
! -------------------------------------------------------------------------------------------------

        SUBROUTINE fcnkb0(t,drop_radius,drdt)
...
!        REAL*8  :: cpm
        ...
        REAL*8, PARAMETER :: r0 = 0.01e-6    ! minimum radius allowed
...
! to omit curvature and solute effects      
        ck = ck * 0.000001
        cr = cr * 0.000001 
        rhol    = rho_w	   ! to use in drdt(1)
! fails        
!        ck = 0.
!        cr = 0.
        
if (radius .ge. r0 .or. s-ck+cr .ge. 0) then    ! works
!if (radius .ge. r0 .or. s .ge. 0) then  		! fails       
        
        denom   = rhol*(Rv*temp/(fbeta*D*es)+Lv**2/(falpha*Ktemp*Rv*temp**2))
!       drdt(1) = 1.0/radius*(s-ck+cr)/denom  ! works
        drdt(1) = 1.0/radius*(s)/denom	      ! works, and completely omits curvature and solute effects
