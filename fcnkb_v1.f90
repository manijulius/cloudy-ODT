...
SUBROUTINE fcnkb(t,drop_radius,drdt)
    ...
    !code block to investigate the floating point error
    !Mani: found that sqrt of temp produces floating point error to catching it before
    !if (temp < 200 .or. temp > 400) then
    !temp = -500
    !if (temp < 0) then
    ...
    ELSE IF(dmaxa .EQ. 2) THEN
      !Ammonium sulphate
       Ms = 132.1395e-3
       !density of Ammonium bisulphate and sulphate are similar so same factor
       c7 = c7am
       nions = 3.0d0
